...
def create_session_menu_keyboard(session_name: str):
    ...
    builder.add(InlineKeyboardButton(text="🎯 Профили закупки", callback_data=f"profiles_{session_name}"))
    ...
    if config:
        for profile_name in config.profiles.keys():
            ...
            builder.add(InlineKeyboardButton(
                ...
                callback_data=f"profile_{session_name}_{profile_name}"
            ))
        
        if len(config.profiles) < 4:
            builder.add(InlineKeyboardButton(text="➕ Добавить профиль", callback_data=f"add_profile_{session_name}"))
...
def create_profile_menu_keyboard(session_name: str, profile_name: str):
    ...
    if config:
        ...
        if config.active_profile != profile_name:
            builder.add(InlineKeyboardButton(
                text="🔸 Активировать", 
                callback_data=f"activate_profile_{session_name}_{profile_name}"
            ))
        
        # Кнопки редактирования стратегий
        builder.add(InlineKeyboardButton(text="1️⃣ Стратегия 1 (Приоритет 1)", callback_data=f"edit_strategy_{session_name}_{profile_name}_1"))
        builder.add(InlineKeyboardButton(text="2️⃣ Стратегия 2 (Приоритет 2)", callback_data=f"edit_strategy_{session_name}_{profile_name}_2"))
        builder.add(InlineKeyboardButton(text="3️⃣ Стратегия 3 (Приоритет 3)", callback_data=f"edit_strategy_{session_name}_{profile_name}_3"))
        builder.add(InlineKeyboardButton(text="4️⃣ Стратегия 4 (Приоритет 4)", callback_data=f"edit_strategy_{session_name}_{profile_name}_4"))
        
        # Кнопка удаления (только для не-дефолтных профилей)
        if profile_name != "default":
            builder.add(InlineKeyboardButton(text="🗑 Удалить профиль", callback_data=f"delete_profile_{session_name}_{profile_name}"))
    
    builder.add(InlineKeyboardButton(text="🔙 Назад", callback_data=f"profiles_{session_name}"))
    builder.adjust(1)
    return builder.as_markup()
    ...
    if callback.message and not isinstance(callback.message, InaccessibleMessage):
        try:
            await callback.message.edit_text(
                ...
                reply_markup=create_sessions_keyboard(callback.from_user.id if callback.from_user else ADMIN_USER_ID),
                parse_mode="HTML"
            )
        except Exception as e:
            logger.error(f"Ошибка при редактировании сообщения: {e}")

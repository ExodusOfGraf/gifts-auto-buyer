"""
Обработчики для редактирования стратегий и админ-функций
"""
...
def register_strategy_callbacks(dp, config_manager, user_contexts, has_access_to_session):
    ...
    async def set_strategy_send_to_channel_prompt(callback: CallbackQuery):
        ...
        text = (
            ...
            f"<b>Формат:</b> <code>-1234567890</code>\n"
            f"(отрицательное число для каналов и групп)\n\n"
            f"💡 Узнать ID канала вы можете отправив сообщение из канала в бота @userinfobot\n\n"
            f"⚠️ <b>Важные требования:</b>\n"
            f"• Канал может быть как приватным, так и публичным\n"
            f"• Аккаунт покупателя должен быть добавлен в канал\n"
            f"• У аккаунта должны быть права на отправку сообщений"
        )
